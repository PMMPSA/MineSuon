		...
		if(!$event->getInstaBreak()){
			do{
				...
				if(!isset($this->breakTimes[$uuid = $player->getRawUniqueId()])){
					$this->getLogger()->debug("⨞§l§c Player " . $player->getName() . " tried to break a block without a start-break action");
					$event->setCancelled();
					break;
				}
				...
				if($actualTime < $expectedTime){
					$this->getLogger()->debug("⨞§l§c Player " . $player->getName() . " tried to break a block too fast, expected $expectedTime ticks, got $actualTime ticks");
					$event->setCancelled();
					break;
				}
