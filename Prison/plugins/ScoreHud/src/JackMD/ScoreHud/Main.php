...
class Main extends PluginBase{
	...
	public function process(Player $player, string $string): string{
		...
		//$string = str_replace("{level}", $this->dataManager->Level($player), $string);
		//$string = str_replace("{maxexp}", $this->dataManager->getMaxExp($player), $string);	
		//$string = str_replace("{exp}", $this->dataManager->getExp($player), $string);		
		//$string = str_replace("{gem}", $this->dataManager->getGem($player), $string);		
		//$string = str_replace("{coin}", $this->dataManager->getPlayerPoint($player), $string);
		//$string = str_replace("{coin}", $this->dataManager->getPlayerCoin($player), $string);
		//$string = str_replace("{coin}", $this->dataManager->getPlayerCoin($player), $string);
		//$this->coin->myCoins($sender->getName())

		if(is_null($this->getServer()->getPluginManager()->getPlugin("EconomyAPI"))) {
		$string = str_replace("{money}", "...", $string);
		}elseif($this->money->myMoney($player) >= 1000000000) {
		$string = str_replace("{money}", round((($this->money->myMoney($player))/1000000000), 2) ."B", $string);
		}elseif($this->money->myMoney($player) >= 1000000) {
		$string = str_replace("{money}", round((($this->money->myMoney($player))/1000000), 2) ."M", $string);
		}elseif($this->money->myMoney($player) >= 1000 ) {
		$string = str_replace("{money}", round((($this->money->myMoney($player))/1000), 2) ."K", $string);
		}elseif($this->money->myMoney($player) <= 1000 ) {
		$string = str_replace("{money}", $this->money->myMoney($player), $string);
		}



		if(is_null($this->getServer()->getPluginManager()->getPlugin("CoinAPI"))) {
		$string = str_replace("{coin}", "...", $string);
		}elseif($this->coin->myCoin($player) >= 1000000000) {
		$string = str_replace("{coin}", round((($this->coin->myCoin($player))/1000000000), 2) ."B", $string);
		}elseif($this->coin->myCoin($player) >= 1000000) {
		$string = str_replace("{coin}", round((($this->coin->myCoin($player))/1000000), 2) ."M", $string);
		}elseif($this->coin->myCoin($player) >= 1000 ) {
		$string = str_replace("{coin}", round((($this->coin->myCoin($player))/1000), 2) ."K", $string);
		}elseif($this->coin->myCoin($player) <= 1000 ) {
		$string = str_replace("{coin}", $this->coin->myCoin($player), $string);
		}
...
/*		if (($fcaClan = Server::getInstance()->getPluginManager()->getPlugin("GCPClan")) !== null) {
		...
		if(!(is_null($this->getServer()->getPluginManager()->getPlugin("GCPClan")))) {
			if($this->getServer()->getPluginManager()->getPlugin("GCPClan")->haveClan($player)) {
			$string = str_replace("{clan}", $this->getServer()->getPluginManager()->getPlugin("GCPClan")->getClanName($player), $string);
		}else{$string = str_replace("{clan}", "...", $string);}
        }else{$string = str_replace("{clan}", "...", $string);}


		//$string = str_replace("{rbcoin}", $this->dataManager->getPlayerRbcoin($player), $string);
