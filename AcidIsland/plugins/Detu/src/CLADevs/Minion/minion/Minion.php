...
class Minion extends Human{
    ...
    public function initEntity(): void{
        ...
        $this->setNameTag("§l§f[§bĐệ Tử§f]§a\n§eCủa §f" . $this->player . "\n§eLevel: §a" . $this->getLevelM());
        ...
        if($source instanceof EntityDamageByEntityEvent){
            ...
            if($damager->getName() !== $this->player){
                if(!$damager->hasPermission("detu.openall")){
                    $damager->sendMessage(C::RED . "§l§f[§aĐệ Tử§f]§bĐây Không Phải Đệ Tử Của Bạn.");
                    return;
                }}
    ...
    public function entityBaseTick(int $tickDiff = 1): bool{
		...
		if($player = Plu::get()->getServer()->getPlayer($this->player)){
            $player->sendPopup("§l§bĐệ tử của bạn yêu cầu cúp kim cương");
            return false;
        }
        }
                }
    ...
    public function checkEverythingElse(): bool{
        ...
        if($damage >= 1560){
            if($player = Plu::get()->getServer()->getPlayer($this->player)){
        $player->sendMessage("§l§f[§aĐệ Tử§f]§b Cúp Của Đệ Tử Sắp Hỏng Hãy Fix Để Đệ Tiếp Tục Mine");
            }
        return false;
        }
        $player = $this->getLevel()->getServer()->getPlayer($this->player);
	...
	$xu = 0;
                ...
                $inventory->remove($item);
						...
						if(Plu::get()->mode($this->player) == "on"){
								    if($player = Plu::get()->getServer()->getPlayer($this->player)){		
                $player->sendMessage("§l§f[§aĐệ Tử§f] §bbạn nhận được §f".$xu." §bxu");          			                EconomyAPI::getInstance()->addMoney($player, $xu);
									}
						}
				}
						}
    if($inventory->canAddItem($drop)){ return true;}elseif(!$inventory->canAddItem($drop)){ 
        if($player = Plu::get()->getServer()->getPlayer($this->player)){
        $player->sendMessage(C::RED . C::BOLD . "§l§f[§aĐệ Tử§f] §b Đệ Của Bạn Không Bán Được Item Trong Chest");
        return false;
        }
        }
            }elseif(!in_array($block->getId(), Plu::get()->getConfig()->getNested("blocks.cannot"))){
    if($player = Plu::get()->getServer()->getPlayer($this->player)){
        $player->sendMessage(C::RED . C::BOLD . "§l§f[§aĐệ Tử§§f] §bĐệ Của Bạn Không Thể Phá Được Block Này");
    }
        return false;
            }
            return false;
    ...
    public function getChestCoordinates(): string{
        if(!isset($this->getCoord()[1])){
            return C::RED . "§l§f[§aĐệ Tử§f]§b Không Tìm Thấy Kết Nối";
        }
