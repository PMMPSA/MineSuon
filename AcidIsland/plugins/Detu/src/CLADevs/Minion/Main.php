          ...
          if($sender->hasPermission("detu.cmd"))
	           ...
	           if(!$p = $this->getServer()->getPlayer($args[0])){
	            ...
	            return false;
	                ...
	                if(isset($args[0])){
                    if(!$p = $this->getServer()->getPlayer($args[0])){
                        $sender->sendMessage("§cKhông Tìm Được Tên Của Người Chơi Này.");
                        return false;
                        }
                    $this->giveItem($p);
                    return false;
           ...
           if(!isset($args[0])){
               $sender->sendMessage("§laSellMessage Thường Dùng: /detusellmessage <on|off>");
               return false;
           }
           switch ($args[0]) {
               case "on":
			       $sender->sendMessage("§l§aSellMessage Bật ");
				   $this->nosell[$sender->getName()] = "on";
				   $sender->getInventory()->setSize(65);
				   break;

               case "off":
			       $sender->sendMessage("§l§aSellMessage tắt "); 
                   $this->nosell[$sender->getName()] = "off";
				   break;
               default :
                   $sender->sendMessage("§l§aSellMessage Thường Dùng: /detusellmessage <on|off>");
                   break;
           }
       }
