...
class Main extends PluginBase {
	...
	public function onEnable() {
		$this->getServer()->getLogger()->info(" §l§aPlugin BuyEC Server Mine Dream Đã Hoạt Động...");
...
public function onCommand(CommandSender $s,Command $cmd,string $label, array $args) : bool{
		if(!$s->hasPermission("server.trade")){
		    return false;
		}
			...
			if(isset($args[0]) && isset($args[1])) {				  
				if(is_numeric($args[0])) {					  
					if(is_numeric($args[1])) {						
						if($this->eco->myMoney($s->getName()) >= 0) {		  
							$enchantLevel = $args[1];
							$enchantId = $args[0];
						...
						$s->sendMessage("• Cấp độ phải là số");
						return false;
						}
				...
				$s->sendMessage("• Cách dùng: /buyec <ID> <cấp độ>");
				return false;
			}
		}
		...
		if($cmd->getName() == "listec") {
		  ...
		  if(isset($args[0])) {
			  ...
			  $s->sendMessage("• Cách dùng: /listec <trang>");
			  return true;
		  }
		}
	}
