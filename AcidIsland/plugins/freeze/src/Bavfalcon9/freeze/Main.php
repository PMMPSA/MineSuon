	...
	public function onCommand(CommandSender $sender, Command $command, string $label, array $args) : bool {
		if($command->getName() === "freeze") {
			...
			if(in_array($player->getName(), $this->frozen)) {
				...
				$player->setImmobile(true);
				$this->getServer()->broadcastMessage($this->freeze ."§a" . $player->getName() . "§r đã bị đóng băng SML!."); // Adding to config soon.
		...
		if($command->getName() === "unfreeze" || $command->getName() === "thaw") {
			...
			if(in_array($player->getName(), $this->frozen)) {
				...
				$sender->sendMessage($this->freeze . "§a" . $player->getName() . "§r không bị đóng băng");
				return true;
			}
			return true;
		}
